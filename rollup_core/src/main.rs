...
fn main() {
    ...
    let asdserver_thread = thread::spawn(|| {
        ...
        // .block_on() Blocks the current thread while running the async code to completion
        rt.block_on(async { sequencer::run(sequencer_receiver, db_sender2).unwrap() });

        // .spawn() Spawns another async task on the runtime, but doesn't wait for it to complete
